...
| `coach.js` `coach-build.js` `coach-live.js` `coach-prog.js` `coach-goal.js` `coach-tags.js` `coach-data.js` `coach-ui.js` | Coach — the deterministic engine that reads the log and says one true thing about it, (`coach-build.js`) the workout builder that turns it into a session he can start, (`coach-live.js`) the in-session read that says what comes next during one, (`coach-prog.js`) the targets — per lift, the weight and reps for next time, from his own history — and (`coach-goal.js`) the goal's dials and the energy context the targets read. `coach.js`, `coach-build.js`, `coach-live.js`, `coach-prog.js`, `coach-goal.js` and `coach-tags.js` are **pure** and are copied into the native tree verbatim; `coach-data.js` is the impure gatherer the port rewrites; `coach-ui.js` is the card, the sheet, the target line and the live session's chip and nudge. Settings live at `settings/coach` (the goal is two answers there, not a key of its own); the greeting counter is device storage, deliberately — see AGENTS.md |
