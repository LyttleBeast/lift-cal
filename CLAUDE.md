...
| `coach.js` `coach-build.js` `coach-tags.js` `coach-data.js` `coach-ui.js` | Coach — the deterministic engine that reads the log and says one true thing about it, and (`coach-build.js`) the workout builder that turns it into a session he can start. `coach.js`, `coach-build.js` and `coach-tags.js` are **pure** and are copied into the native tree verbatim; `coach-data.js` is the impure gatherer the port rewrites; `coach-ui.js` is the card and the sheet. Settings live at `settings/coach`; the greeting counter is device storage, deliberately — see AGENTS.md |
