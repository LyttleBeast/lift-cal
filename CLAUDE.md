...
| `coach.js` `coach-build.js` `coach-live.js` `coach-prog.js` `coach-goal.js` `coach-overlap.js` `coach-tags.js` `coach-data.js` `coach-ui.js` | Coach — the deterministic engine that reads the log and says one true thing about it, (`coach-build.js`) the workout builder that turns it into a session he can start, (`coach-live.js`) the in-session read that says what comes next during one, (`coach-prog.js`) the targets — per lift, the weight and reps for next time, from his own history — (`coach-goal.js`) the goal's dials, the energy context, bodyweight at a moment and the goal's pace, and (`coach-overlap.js`) stage two: plateau or cut, the stall ladder, the lighter week, the record day, and the stage-three reads that replay a target or read a lift's series. `coach.js`, `coach-build.js`, `coach-live.js`, `coach-prog.js`, `coach-goal.js`, `coach-overlap.js` and `coach-tags.js` are **pure** and are copied into the native tree verbatim; `coach-data.js` is the impure gatherer the port rewrites; `coach-ui.js` is the card (an earned line, since v49), the sheet, the target line, Your goal and the live session's chip and nudge. Settings live at `settings/coach` (the goal is answers there plus one `goalLift` key); the greeting counter and the card's line memory are device storage, deliberately — see AGENTS.md |
