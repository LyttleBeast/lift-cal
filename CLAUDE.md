...
| `coach.js` `coach-build.js` `coach-live.js` `coach-prog.js` `coach-goal.js` `coach-overlap.js` `coach-ready.js` `coach-fuel.js` `coach-volume.js` `coach-tags.js` `coach-data.js` `coach-ui.js` | Coach — the deterministic engine that reads the log and says one true thing about it, (`coach-build.js`) the workout builder that turns it into a session he can start, (`coach-live.js`) the in-session read that says what comes next during one — since v54 counting the sessions finished earlier that day, and saying the next set and the effort chips' words — (`coach-prog.js`) the targets — per lift, the weight and reps for next time, from his own history, and since v52 `targetFor()`, which gives a marked session's lift the target from before it, and since v54 his per-set effort rating and `nextSet()`, the next set mid-session — (`coach-goal.js`) the goal's dials, the energy context, bodyweight at a moment and the goal's pace, and (`coach-overlap.js`) stage two: plateau or cut, the stall ladder, the lighter week, the record day, and the stage-three reads that replay a target or read a lift's series, (`coach-ready.js`) stage four's training half: recovery windows, the rest read, readiness, what was different about a session, and (`coach-fuel.js`) its food half, "Am I fueled?" — the two never import each other — and (`coach-volume.js`, v54) stage five's whole week: each group's hard sets against a common range and his own normal (since v56 strength's 6–15 only on the main lifts' groups, `coach-goal.js` `MAIN_LIFTS`), the neglected group, and balance, counts only — since v56 counting a custom exercise by the movement he sets on it (`coach-tags.js` `ownMovement()`). `coach.js`, `coach-build.js`, `coach-live.js`, `coach-prog.js`, `coach-goal.js`, `coach-overlap.js`, `coach-ready.js`, `coach-fuel.js`, `coach-volume.js` and `coach-tags.js` are **pure** and are copied into the native tree verbatim; `coach-data.js` is the impure gatherer the port rewrites; `coach-ui.js` is the card (an earned line, since v49; after a workout, since v53, the finish line — `finishRead()` in `coach.js`, which the recap and the sheet read too — and a warm line when nothing is earned), the sheet, the target line, Your goal and the live session's chip and nudge. Settings live at `settings/coach` (the goal is answers there plus one `goalLift` key, the bad-day `marks` since v52, since v54 the neglect line's `asked.vol_neglect` stamp, and since v56 the balance answer's `asked.bal_custom`); the food days "Am I fueled?" reads are read on an ask, Pro only, never on a paint; the greeting counter and the card's line memory are device storage, deliberately — see AGENTS.md |
| `food.js` `recall.js` `importer.js` `ai.js` `ai-config.js` `estimate-origin.js` `estimate-ask.js` | Fuel (`estimate-origin.js` says where each estimated number came from and, since v55, names a meal saved off the estimate sheet; `estimate-ask.js`, v55, is the pure client half of the estimator's "Which one?" question. Both import nothing and are copied into the native tree verbatim) |
| `workout.js` `routines.js` `blocks.js` `exercises.js` `picker.js` | Train (`blocks.js` is the pure lifting-block model, shared by the session screen and the routine editor; since v53 the recap leads with the finish line and asks "How did that feel?", which writes a session's `feel`; since v54 a hand-added exercise shows last time's numbers in grey, and a set can carry his effort rating, `rir`; since v55 a drop set's drops sit under it, each a `'D'` with `dp: 1`, and every edit to a list of sets goes through `analytics.js`'s four drop-set edits — see AGENTS.md; since v56 `+ Set` after a drop set copies its first set, the routine editor draws drop sets grouped with **+ Drop**, and a custom exercise can carry its movement, `exercises/custom[i].pattern` and `.angle`, set in `picker.js`) |
| `weight.js` `weightmodel.js` `tdee.js` | Weight, trend, maintenance |
| `water.js` `steps.js` | Water and Steps |
| `stats.js` `analytics.js` `ui.js` | Shared stats, charts, UI helpers (`analytics.js` holds the drop-set rule since v55, and since v56 `repeatOf()`, the set `+ Set` copies) |
