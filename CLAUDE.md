...
| `coach.js` `coach-build.js` `coach-live.js` `coach-prog.js` `coach-goal.js` `coach-overlap.js` `coach-ready.js` `coach-fuel.js` `coach-tags.js` `coach-data.js` `coach-ui.js` | Coach — the deterministic engine that reads the log and says one true thing about it, (`coach-build.js`) the workout builder that turns it into a session he can start, (`coach-live.js`) the in-session read that says what comes next during one, (`coach-prog.js`) the targets — per lift, the weight and reps for next time, from his own history, and since v52 `targetFor()`, which gives a marked session's lift the target from before it — (`coach-goal.js`) the goal's dials, the energy context, bodyweight at a moment and the goal's pace, and (`coach-overlap.js`) stage two: plateau or cut, the stall ladder, the lighter week, the record day, and the stage-three reads that replay a target or read a lift's series, (`coach-ready.js`) stage four's training half: recovery windows, the rest read, readiness, what was different about a session, and (`coach-fuel.js`) its food half, "Am I fueled?" — the two never import each other. `coach.js`, `coach-build.js`, `coach-live.js`, `coach-prog.js`, `coach-goal.js`, `coach-overlap.js`, `coach-ready.js`, `coach-fuel.js` and `coach-tags.js` are **pure** and are copied into the native tree verbatim; `coach-data.js` is the impure gatherer the port rewrites; `coach-ui.js` is the card (an earned line, since v49; after a workout, since v53, the finish line — `finishRead()` in `coach.js`, which the recap and the sheet read too — and a warm line when nothing is earned), the sheet, the target line, Your goal and the live session's chip and nudge. Settings live at `settings/coach` (the goal is answers there plus one `goalLift` key, and the bad-day `marks` since v52); the food days "Am I fueled?" reads are read on an ask, Pro only, never on a paint; the greeting counter and the card's line memory are device storage, deliberately — see AGENTS.md |
| `food.js` `recall.js` `importer.js` `ai.js` `ai-config.js` | Fuel |
| `workout.js` `routines.js` `blocks.js` `exercises.js` `picker.js` | Train (`blocks.js` is the pure lifting-block model, shared by the session screen and the routine editor; since v53 the recap leads with the finish line and asks "How did that feel?", which writes a session's `feel` — see AGENTS.md) |
