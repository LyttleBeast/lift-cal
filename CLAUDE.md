...
| `workout.js` `routines.js` `blocks.js` `exercises.js` `picker.js` | Train (`blocks.js` is the pure lifting-block model, shared by the session screen and the routine editor; since v53 the recap leads with the finish line and asks "How did that feel?", which writes a session's `feel`; since v54 a hand-added exercise shows last time's numbers in grey, and a set can carry his effort rating, `rir` — see AGENTS.md) |
