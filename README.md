...
| `food.js` | Fuel tab (v55: **Save as meal** on the estimate sheet, and the "Which one?" question; v56: **Save as meal** on "Found in your log" too; v57: the note under the calorie bar says where the target sits against the holding band, with the band's numbers — `targetNote()`; v58: *Reading the bar* says what the yellow comes to a week — `holdWords()`, `markWords()` — and a row's fix-it sheet shows a generic's USDA description) |
| `estimate-origin.js` | What the estimate sheet says about where each number came from — the menu, published nutrition, or an estimate — and (v55) the name a meal saved off it starts with, and (v58) a USDA row's own description, `src.desc`, for the fix-it sheet. Pure, imports nothing; copied verbatim |
