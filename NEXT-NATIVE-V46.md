...
And three fixes Micah found on his phone walking v45 (Phase 0).
...
**What is stored:** one new key, `settings/coach.on` — `{ patterns: true }` or
absent — and it is the Patterns switch. Nothing else. See §6: the PUBLISHED
rules take it as they are; the PROPOSED ones in `web-patches/` do NOT.
...
**NO PINNED FILE MOVED**, and neither did `coach-build.js`
(`a378c274a5eb07ac3acc11a11bf13fb78505bf444a7bf1c0408b83cb28036379`, rack-v45's
byte for byte). The files the port copies verbatim that did change or arrive:

```
coach.js       eeb430102d70118ba20b95499d914fbae757d079954ed15419a2321932c7c1dd   CHANGED — re-copy, 2,787 lines
...
- imports `liveRead` and `LIVE_NONE` from `./coach-live.js`, and re-exports
  `LIVE_NONE` (the sheet's words for "nothing to add")
...
The block check box is untouched: it ticks only sets that already have reps, so
it can never make a set Finish would drop.
...
| Settings → Coach — a Patterns switch, OFF by default | `src/ui/coach/settings.jsx` | if it walks `CATEGORIES`, the row appears by itself — but the switch must read `!isMuted(settings, id)`, not `!settings.mute[id]`, or it shows Patterns ON |
...
## 6. `settings/coach.on` — the PUBLISHED rules take it; the PROPOSED ones do not
...
| `coach-surface.mjs` | + G (the chip, the sheet, Add it, the line's appear-once rule, Basic and edit see nothing) and H (the Patterns switch and bubble). **Does not port** — native wants its own, and G found an engine bug no engine verifier did. |
